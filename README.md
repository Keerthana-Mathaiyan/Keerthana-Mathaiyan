<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=28&duration=3000&color=00C2FF&center=true&vCenter=true&width=800&lines=Data+Analytics+Professional;5+Years+Enterprise+Experience;SQL+%7C+Python+%7C+Power+BI;Machine+Learning+%7C+NLP" />
</p>

<h1 align="center">Hi 👋, I'm Keerthana</h1>
<h3 align="center">Data & Analytics Professional | Ex-TCSer | SQL | Python | Power BI | Machine Learning</h3>

---

## 🚀 About Me

- 📊 5 years of enterprise analytics experience at TCS  
- 🧠 Certified in Data Science (ML, DL, NLP)  
- 🔎 Strong expertise in SQL, Python & Power BI  
- 🎯 Focused on solving real-world business problems using data  

I specialize in transforming raw business data into strategic insights and predictive solutions.

---

## 🛠 Tech Stack

### 📊 Data & BI
- SQL (Advanced Queries, CTEs, Window Functions)
- Power BI (DAX, Data Modeling, KPI Dashboards)
- Excel (Advanced Analytics)

### 🐍 Programming
- Python  
  - Pandas  
  - NumPy  
  - Matplotlib / Seaborn  
  - Scikit-learn  

### 🤖 Machine Learning
- Supervised & Unsupervised Learning
- Feature Engineering
- Model Evaluation
- Basic Deep Learning
- NLP Fundamentals

---

## 📌 Featured Projects

### 🔹 Customer Churn Prediction
End-to-end ML pipeline to predict customer churn and improve retention strategy.

**Tech:** Python, Scikit-learn, Pandas  

---

### 🔹 Business SQL Case Studies
Solved enterprise-style business problems:
- Revenue Analysis
- Cohort Analysis
- KPI Reporting
- Customer Segmentation  

**Tech:** SQL  

---

### 🔹 Power BI Executive Dashboard
Interactive dashboards tracking:
- Sales KPIs
- Regional Performance
- Customer Behavior  

**Tech:** Power BI, DAX  

---

### 🔹 NLP Sentiment Analysis
Analyzed product reviews to extract customer sentiment and business insights.

**Tech:** Python, NLP  

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Keerthana-Mathaiyan&show_icons=true&theme=tokyonight" width="48%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Keerthana-Mathaiyan&theme=tokyonight" width="48%" />
</p>

---

## 📫 Connect With Me

- 💼 LinkedIn: [[Keerthana Mathaiyan](https://www.linkedin.com/in/keerthana-mathaiyan/)]
- 🌐 Portfolio: [[Keerthana-Mathaiyan](https://github.com/Keerthana-Mathaiyan)]

---

⭐ Open to Data Analyst / Data Scientist opportunities.
<!--
**Keerthana-Mathaiyan/Keerthana-Mathaiyan** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
